...
/**
 * The guard below makes this migration safe to re-run. See payload.config.ts:
 * `prodMigrations` applies pending migrations on boot, and a schema created
 * without a `payload_migrations` row would otherwise be built a second time.
 */
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  const applied = await db.execute(sql`select to_regclass('public.articles') as present`)
  if (applied.rows[0]?.present) {
    payload.logger.info('Skipping: the articles tables are already present.')
    return
  }
  ...
  CREATE TYPE "public"."enum__articles_v_published_locale" AS ENUM('en');
  ...
  ALTER TABLE "articles_page" ADD CONSTRAINT "articles_page_seo_image_id_media_id_fk" FOREIGN KEY ("seo_image_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "articles_page_seo_seo_image_idx" ON "articles_page" USING btree ("seo_image_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  ALTER TABLE "articles_page_locales" DISABLE ROW LEVEL SECURITY;
